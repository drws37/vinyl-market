...
router.post('/', async (req, res) => {
  try { 
    const {id} = req.body
    if (res.locals.user){
        const orderCheck = await Order.findOne({where: {user_id:res.locals.user.id, status:'Корзина'}})
        if (orderCheck){
          const record = await Record.findOne({where: id})
          const orderItem  =await OrderItem.findOne({where: {order_id:orderCheck.id, record_id: record.id}})
          let orderItemCreate
          if(orderItem){
