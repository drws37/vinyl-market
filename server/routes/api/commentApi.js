...
router.post('/comment', async (req, res) => {
	try {
		...
		if (res.locals.user) {
			const commentAdd = await SellersComment.create({
				seller_id: +seller_id,
				comment,
				user_id: res.locals.user.id,
			});
			const commentUser = await SellersComment.findOne({
				include: { model: User },
...
router.get('/comments/:id', async (req, res) => {
	try {
		...
		const comments = await SellersComment.findAll({
			include: { model: User },
...
router.delete('/:comId', async (req, res) => {
	...
	try {
		if (
			...
			comment.user_id === res.locals.user.id
		) {
