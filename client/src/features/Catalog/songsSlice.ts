...
export const songsAdd = createAsyncThunk('songs/add', async (obj: { songs: SongWithoutId[] }) =>
  api.fetchSongsAdd(obj),
);
export const songsLoad = createAsyncThunk('songs/load', () => api.fetchSongsLoad());
export const songsDelete = createAsyncThunk('songs/delete', (id: SongId) =>
  api.fetchSongDelete(id),
);
...
const songsSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(songsDelete.fulfilled, (state, action) => {
        state.songs = state.songs.filter((song) => song.id !== action.payload);
      })
      .addCase(songsDelete.rejected, (state, action) => {
        state.message = action.error.message;
      });
  },
});
